...
public static class Stop2BarCounterSetup
{
    private const string StopName = "Stop_2";
    private const string VisualName = "Stop 2 Bar Counter Visual";
    private const string ModelPath = "Assets/Models/Stops/Bar_Counter_Stop.fbx";

    [MenuItem("Game/Setup/Apply Stop 2 Bar Counter")]
    public static void ApplyStop2BarCounter()
    {
        ...
        {
            Debug.LogError($"Could not apply Stop 2 bar counter visual. Stop: {stop}, model: {modelPrefab}");
            return;
        }
    ...
    [MenuItem("Game/Setup/Cleanup Stop 2 Bar Counter")]
    public static void CleanupStop2BarCounter()
    {
        GameObject stop = GameObject.Find(StopName);
        if (stop == null)
        {
