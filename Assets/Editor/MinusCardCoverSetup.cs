...
public static class MinusCardCoverSetup
{
    private const string StackName = "Card Stack 1";
    private const string VisualName = "Minus Card Cover Visual";
    private const string ModelPath = "Assets/Models/Cards/Minus_Card_Cover.fbx";

    [MenuItem("Game/Setup/Apply Minus Card Covers")]
    public static void ApplyMinusCardCovers()
    {
        ...
        {
            Debug.LogError($"Could not apply minus card covers. Stack: {stack}, model: {modelPrefab}");
            return;
        }
    ...
    [MenuItem("Game/Setup/Cleanup Minus Card Covers")]
    public static void CleanupMinusCardCovers()
    {
        GameObject stack = GameObject.Find(StackName);
        if (stack == null)
        {
    ...
    private static void FitVisualToCard(GameObject visual, Bounds? targetBounds)
    {
        ...
        {
            Debug.LogError($"Minus card cover has no renderers: {visual.name}");
            return;
        }
